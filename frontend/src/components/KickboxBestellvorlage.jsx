...
const SIZES = {
  'kickbox': [100, 110, 120, 130, 140, 150, 'S', 'M', 'L', 'XL', 'XXL'],
};

const SIZES_KIDS  = [100, 110, 120, 130, 140, 150];
const SIZES_ADULT = ['S', 'M', 'L', 'XL', 'XXL'];

// Maßtabelle: alle Größen der Hersteller-Sizechart (Kinder-cm + Erwachsenen-Buchstaben)
const MASS_SIZES = [100, 110, 120, 130, 140, 150, 'S', 'M', 'L', 'XL', 'XXL'];

// Hersteller-Sizechart Kickboxanzug — 1:1 aus Vorlage (Toleranz ±1,0 cm)
// A = Oberteil-Länge · B = Oberteil-Breite · C = Hosenlänge gesamt · D = Bundbreite · E = Innenbeinlänge
const KICKBOX_SIZECHART = {
  100:   { A: '49', B: '40', C: '85',  D: '27',   E: '67' },
  110:   { A: '52', B: '42', C: '87',  D: '28',   E: '69' },
  120:   { A: '55', B: '45', C: '91',  D: '29',   E: '70' },
  130:   { A: '58', B: '48', C: '94',  D: '30',   E: '75' },
  140:   { A: '61', B: '50', C: '98',  D: '32',   E: '76' },
  150:   { A: '64', B: '54', C: '100', D: '32.5', E: '80' },
  'S':   { A: '67', B: '55', C: '106', D: '33',   E: '84' },
  'M':   { A: '71', B: '59', C: '109', D: '34',   E: '86' },
  'L':   { A: '73', B: '63', C: '113', D: '35',   E: '88' },
  'XL':  { A: '76', B: '67', C: '117', D: '37',   E: '89' },
  'XXL': { A: '81', B: '71', C: '120', D: '38',   E: '93' },
};
// Vorbefüllte Maßtabelle aus der Sizechart (Default für neue Bestellungen)
const DEFAULT_MASS = MASS_SIZES.reduce((acc, s) => ({ ...acc, [s]: { ...KICKBOX_SIZECHART[s] } }), {});

const EMPTY_MENGEN = (model) =>
  (SIZES[model] || SIZES['kickbox']).reduce((acc, s) => ({ ...acc, [s]: '' }), {});
...
const POSITIONEN = [
  'Brust links', 'Brust rechts', 'Rücken oben', 'Rücken Mitte',
  'Linker Ärmel', 'Rechter Ärmel', 'Innenseite Jacke', 'Hosenbein links', 'Hosenbein rechts',
];
const MATERIALIEN  = ['100% Baumwolle', 'Baumwolle/Polyester', 'Polyester', 'Canvas', 'Synthetik'];
...
// Maßpunkte gemäß Hersteller-Sizechart Kickboxanzug (Oberteil A/B · Hose C/D/E)
const MASSPUNKTE = [
  { key: 'A', num: 'A', label: 'Oberteil-Länge',    hint: 'Schulter bis Saum Oberteil' },
  { key: 'B', num: 'B', label: 'Oberteil-Breite',   hint: 'Brustbreite, flach gemessen' },
  { key: 'C', num: 'C', label: 'Hosenlänge gesamt', hint: 'Bund bis Hosensaum' },
  { key: 'D', num: 'D', label: 'Bundbreite',        hint: 'Breite Hosenbund flachgelegt' },
  { key: 'E', num: 'E', label: 'Innenbeinlänge',    hint: 'Schritt bis Hosensaum' },
];
...
export default function GiBestellvorlage({ artikel = null, vorlage = null, onClose = null, initEditingId = null, initFormdata = null, overrideDojoId = null }) {
  ...
  // Produktbild lokal hochladen (Base64 im Formular) — funktioniert auch ohne gespeicherte Vorlage
  const handleProduktImagePick = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = ev => setForm(p => ({ ...p, produkt_image: ev.target.result }));
    reader.readAsDataURL(file);
    e.target.value = '';
  };
  ...
  const switchModel = (model) => {
    const oldSizes = SIZES[form.model] || SIZES['kickbox'];
    const newSizes = SIZES[model] || SIZES['kickbox'];
    const migrate  = (old) => {
      const next = {};
  ...
  // Maße auf Hersteller-Sizechart zurücksetzen
  const resetMasse = () =>
    setForm(p => ({ ...p, spezifikation: { ...p.spezifikation, massTabelle: MASS_SIZES.reduce((acc, s) => ({ ...acc, [s]: { ...KICKBOX_SIZECHART[s] } }), {}) } }));
  ...
  const generatePdf = async (lang = 'de', printWin = null, forcedDojoId = null) => {
    ...
    try {
      ...
      const html = buildKickboxPdf(form, origin, eingebetteteDateien, neueBestellungId, lang);
      ...
      <div className="gv-header">
        <div>
          ...
          <div className="gv-title">
            {vorlage ? vorlage.name : artikel ? `Bestellvorlage: ${artikel.name}` : 'Kickboxanzug Bestellvorlage'}
          </div>
          {editingBestellungId ? (
        ...
        <div style={{ display: 'flex', gap: '0.6rem', alignItems: 'center', flexShrink: 0 }}>
          <span style={{ fontSize: '0.7rem', opacity: 0.45, fontFamily: 'monospace', flexShrink: 0 }}>Kickbox v1.0</span>
          {draftMsg && (
      ...
      <div className="gv-body">
        ...
        <div className="gv-section">
          ...
          <div className="gv-model-row">
            <div className="gv-model-card active" style={{ cursor: 'default', maxWidth: '200px' }}>
              <div className="gv-model-name">Kickboxanzug</div>
              <div className="gv-model-detail">11 Größen · 100–150 cm + S–XXL</div>
              <div className="gv-model-hint">Oberteil + Hose nach Sizechart</div>
            </div>
            ...
            {/* Produktbild — lokaler Upload (Base64), auch ohne gespeicherte Vorlage. Die Sizechart steht unten in der Maßspezifikation. */}
            {(() => {
              const serverImg = dateien.find(d => d.tag === '__produktbild__');
              const imgSrc = form.produkt_image || serverImg?.pfad || null;
              const removeImg = () => {
                if (form.produkt_image) setForm(p => ({ ...p, produkt_image: '' }));
                else if (serverImg) deleteDatei(serverImg.datei_id);
              };
              return (
                <div className="gv-produkt-preview-wrap">
                  <div className="gv-produkt-img-box">
                    {imgSrc ? (
                      ...
                      <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.35rem', width: '100%', height: '100%', minHeight: '120px', cursor: 'pointer', color: 'var(--ds-text-faint)', fontSize: '0.75rem', textAlign: 'center', border: '1px dashed rgba(255,255,255,0.18)', borderRadius: '8px', padding: '0.75rem' }}>
                        <span style={{ fontSize: '1.5rem' }}>📷</span>
                        <span>Produktbild<br />hochladen</span>
                        <input type="file" accept="image/*" style={{ display: 'none' }} onChange={handleProduktImagePick} />
                      </label>
                    )}
                    {imgSrc && (
                      <button className="gv-produkt-del" onClick={removeImg} title="Entfernen">×</button>
                    )}
                  </div>
                  {imgSrc && (
                    <label className="gv-produkt-add-btn" title="Bild ersetzen" style={{ cursor: 'pointer' }}>
                      ↑
                      <input type="file" accept="image/*" style={{ display: 'none' }} onChange={handleProduktImagePick} />
                    </label>
                  )}
        ...
        <div className="gv-section">
          ...
          <div className="gv-grid2">
            <div className="gv-field">
              <label className="gv-label">Material <InfoBtn text="Grundmaterial des Kickboxanzugs. Exakte Zusammensetzung im Textfeld angeben." /></label>
        ...
        <div className="gv-section">
          <div className="gv-section-title-row">
            ...
            <button className="gv-btn-proportional" onClick={resetMasse}
              title="Maßtabelle auf die Hersteller-Sizechart zurücksetzen">
              ↺ Sizechart zurücksetzen
            </button>
            {vorlage?.vorlage_id && (
          ...
          <div style={{ fontSize: '0.73rem', color: 'var(--ds-text-faint)', marginBottom: '0.65rem' }}>
            Alle Angaben in cm · Toleranz ±1,0 cm · A=Oberteil-Länge · B=Oberteil-Breite · C=Hosenlänge gesamt · D=Bundbreite · E=Innenbeinlänge · Werte aus der Hersteller-Sizechart, bei Bedarf überschreibbar
          </div>
          <div className="gv-mass-layout">
            {zeichnungSichtbar && (
              <img
                className="gv-zeichnung-img"
                src={form.zeichnung_image || '/kickbox-charts/sizechart-kickbox.jpg'}
                alt="Maßzeichnung"
              />
            )}
        ...
        <div className="gv-section">
          ...
          {/* Positionen — Jacke */}
          <div className="gv-pos-cat-label">Oberteil</div>
        ...
        <div className="gv-section">
          ...
          <div className="gv-field" style={{ marginTop: '0.65rem' }}>
            ...
            <input className="gv-input" value={spez.verp_label || ''} onChange={fSpez('verp_label')} placeholder="z. B. KICKBOXANZUG · Gr. XXX · Art. Nr. XXXX · Kampfkunstschule Schreiner" />
          </div>
...
//  PDF-HTML-GENERATOR
// ═══════════════════════════════════════════════════════
export function buildKickboxPdf(form, origin, eingebetteteDateien = [], bestellungId = null, lang = 'de') {
  // Bilingual PDF: alle Labels immer auf Deutsch und Englisch
  const T = {
    docTitle:       'Kickboxanzug Bestellvorlage – Kampfkunstschule Schreiner',
    h1:             'Kickboxanzug',
    sub:            'Bestellvorlage · Kampfkunstschule Schreiner  ·  Order Form · Martial Arts School Schreiner',
    nr:             'Nr. / No.',
    datum:          'Datum / Date:',
    pageHeader:     'Kickboxanzug Bestellvorlage / Order Form – Kampfkunstschule Schreiner',
    tolerance:      'Toleranz ±1,0 cm / Tolerance ±1.0 cm',
    ...
    s9sub: 'Kickboxanzug – Hersteller-Sizechart / Reference Size Chart',
    // Fields
    ...
    refPoints:   'A=Oberteil-Länge/Top length · B=Oberteil-Breite/Top width · C=Hosenlänge gesamt/Trouser length · D=Bundbreite/Waistband width · E=Innenbeinlänge/Inseam',
  ...
  const POS_LABEL = {
    'Brust links':'Brust links / Left Chest','Brust rechts':'Brust rechts / Right Chest',
    ...
    'Innenseite Jacke':'Innenseite Jacke / Jacket Inside',
  ...
  const MP_EN = {
    A: 'Top length', B: 'Top width', C: 'Trouser length',
    D: 'Waistband width', E: 'Inseam',
  };

  const img188 = form.zeichnung_image || `${origin}/kickbox-charts/sizechart-kickbox.jpg`;
...
${(() => {
  ...
  const pbSrc = form.produkt_image || (pb ? pb.dataUrl : null);
  return `
<div class="sec">
  ...
  <div style="display:flex;gap:6mm;align-items:flex-start;">
    <div style="flex:1;min-width:0;">
      <div class="mc-row" style="align-items:flex-start;margin-bottom:4mm;">
        <div class="mc sel"><div class="mc-n">Kickboxanzug</div><div class="mc-d">11 Größen / 11 Sizes · 100–150 cm + S–XXL</div></div>
      </div>
    ...
    ${pbSrc ? `<div style="flex-shrink:0;text-align:center;">
...
<div class="sec">
  ...
  <div class="fg2">
    ${allTags(T.material, ['100% Baumwolle','Baumwolle/Polyester','Polyester','Canvas','Synthetik'], spez.material||[], m => ({'100% Baumwolle':T.cotton,'Baumwolle/Polyester':T.cottonPoly,'Polyester':'Polyester','Canvas':T.canvas,'Synthetik':T.synthetik}[m]||m))}
    ${fshow(T.materialText || 'Material-Zusammensetzung / Material Composition', spez.materialText)}
  </div>
</div>
...
<div class="sec">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:3mm;margin-bottom:5mm;">
    ${['Brust links','Brust rechts','Rücken oben','Rücken Mitte','Linker Ärmel','Rechter Ärmel','Innenseite Jacke','Hosenbein links','Hosenbein rechts'].map(pos => {
...
<div class="chart-block" style="margin-bottom:4mm;">
  <div class="chart-label" style="border-color:var(--gold);">
    Kickboxanzug &nbsp;·&nbsp; 11 Größen / 11 Sizes (100–150 cm + S–XXL)
  </div>
  <img src="${img188}" alt="Sizechart Kickboxanzug" style="max-height:120mm;">
</div>
