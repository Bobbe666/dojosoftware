...
const MemberDashboard = () => {
  ...
  // Gutscheine + Marketing-Artikel → ausgelagert in <MemberShopWidgets/>
  // (eigener State + eigene Fetches, memoized → entlastet diesen Monolithen).
  ...
  // News → ausgelagert in <MemberNewsWidget/> (eigener State + Fetch, memoized)
  ...
  const [umfrageResults, setUmfrageResults] = useState({});

  // Vertragsstatus/Ruhepause/Kündigung/Anpassung → ausgelagert in <MemberContractStatus/>
  ...
  const downloadApplePass = useCallback(async () => {
    ...
    setWalletError(null);
    try {
      const res = await fetchWithAuth(`${config.apiBaseUrl}/wallet/apple-pass`);
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        setWalletError(data.error || 'Fehler beim Generieren des Passes');
        return;
      }
      ...
      a.href = url;
      a.download = 'mitgliedskarte.pkpass';
      a.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      setWalletError('Verbindungsfehler');
    } finally {
  ...
  // (Vertrag-Anpassungen/Ruhepause/Kündigung-Loader → in <MemberContractStatus/> verlagert)
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* Mobile Hero: Check-in prominent + QR / Abwesenheit sekundär — GANZ OBEN */}
      ...
      {/* Today at a glance — kompakte Übersicht */}
      {memberData && (
      ...
      {popupUmfragen.map(u => {
        ...
        return (
          <div key={u.id} className="md-anim-fade-in-up" style={{
            margin: '0.75rem 1rem 0',
            background: isSuccess ? 'rgba(34,197,94,0.08)' : 'rgba(99,102,241,0.1)',
            border: `1.5px solid ${isSuccess ? 'rgba(34,197,94,0.35)' : 'rgba(99,102,241,0.4)'}`,
            borderRadius: 14, padding: '1rem 1.1rem',
            transition: 'background 0.5s, border-color 0.5s',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              <span style={{ fontSize: '1rem' }}>{isSuccess ? '✅' : '📋'}</span>
              <span style={{ fontWeight: 700, fontSize: '0.8rem', color: isSuccess ? '#86efac' : '#a5b4fc', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                {isSuccess ? 'Danke für deine Teilnahme' : 'Offene Umfrage'}
              </span>
            </div>
            ...
            {isSuccess && res && (
              <div style={{ marginBottom: 8 }}>
                {res.myAnswer && (
                  <div style={{ fontSize: '0.85rem', color: '#86efac', marginBottom: 8, fontWeight: 600 }}>
      ...
      {memberData && (
        <div className="member-stats-grid md-stats-grid">
          ...
          <div className="mitgliedsausweis-container">
            ...
            {/* Apple Wallet Button */}
            <button
              className={`apple-wallet-btn${walletLoading ? ' loading' : ''}`}
              onClick={downloadApplePass}
              disabled={walletLoading}
              title="Mitgliedskarte zu Apple Wallet hinzufügen"
            >
              {walletLoading ? (
                <span className="wallet-btn-text">Wird generiert...</span>
              ) : (
                <>
                  <svg className="wallet-apple-icon" viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
                    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
                  </svg>
                  <span className="wallet-btn-text">Zur Wallet hinzufügen</span>
                </>
              )}
            </button>
            {walletError && (
              <p className="wallet-error-hint">{walletError}</p>
            )}
          </div>
      ...
      {/* Vertragsstatus / Ruhepause / Kündigung / Anpassung → ausgelagert (memoized) */}
      <MemberContractStatus mitgliedId={user?.mitglied_id || memberData?.mitglied_id} />
      ...
      {memberData && (
        <div className="md-info-section">
          ...
          <div className="md-info-list">
            ...
            {/* Gutscheine + Marketing-Artikel → ausgelagert in <MemberShopWidgets/> (memoized) */}
            <MemberShopWidgets mitgliedId={memberData?.mitglied_id || user?.mitglied_id} dojoId={activeDojo?.id || memberData?.dojo_id} />
