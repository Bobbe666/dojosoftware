// =====================================================================================
// WHATSAPP CONVERSATION LIST - Seitenleiste mit WhatsApp-Konversationen
// =====================================================================================
...
const WhatsAppConversationList = ({ activeRoomId, onSelectRoom }) => {
  const { token } = useAuth();
  ...
  useEffect(() => {
    loadConversations();
    // Alle 30 Sekunden neu laden (kein Socket.io für die WhatsApp-Liste)
  ...
  const filteredConversations = filter
    ? conversations.filter(c =>
        c.wa_name?.toLowerCase().includes(filter.toLowerCase()) ||
  ...
  const handleSelectConversation = (conv) => {
    const roomObj = {
      ...
      name: conv.wa_name || 'WhatsApp-Nutzer',
      type: 'direct',
      source: 'whatsapp',
      wa_id: conv.wa_id,
    ...
    return (
      <div className="chat-room-list">
        <div className="chat-room-list__header">
          <span className="chat-room-list__title">💬 WhatsApp</span>
        </div>
    ...
    <div className="chat-room-list">
      ...
      {filteredConversations.length === 0 ? (
        <div className="chat-room-list__empty">
          {filter ? 'Keine Ergebnisse.' : 'Noch keine WhatsApp-Nachrichten eingegangen.'}
        </div>
      ) : (
        <div className="chat-room-list__rooms">
          {filteredConversations.map(conv => (
            ...
            >
              {/* Avatar */}
              <div className="chat-room-item__avatar whatsapp-avatar">
                <span>{getInitials(conv.wa_name)}</span>
              </div>

              {/* Inhalt */}
              <div className="chat-room-item__content">
                <div className="chat-room-item__top">
                  <span className="chat-room-item__name">{conv.wa_name || 'WhatsApp-Nutzer'}</span>
                ...
                <div className="chat-room-item__bottom">
                  <span className="chat-room-item__preview">
                    {conv.last_sender_type !== 'whatsapp_user' && (
