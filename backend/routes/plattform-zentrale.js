...
// ── Zentraler Kalender ────────────────────────────────────────────────────────
// Aggregation extrahiert nach services/kalenderAggregation.js (auch vom
// täglichen Briefing genutzt — briefingService.js)
router.get('/kalender', async (req, res) => {
  const { collectKalenderEintraege } = require('../services/kalenderAggregation');
  ...
  try {
    const { events, meta } = await collectKalenderEintraege(von, bis);
    res.json({ success: true, events, meta });
  } catch (err) {
    logger.error('Kalender-Aggregation Fehler', { error: err.message });
