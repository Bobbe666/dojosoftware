...
router.get("/", (req, res) => {
    ...
    if (stil) {
        const query = `
            SELECT DISTINCT m.mitglied_id, m.vorname, m.nachname, m.strasse as adresse, m.hausnummer, m.plz, m.ort, m.email, m.telefon_mobil
            FROM mitglieder m
        ...
        const query = `
            SELECT mitglied_id, vorname, nachname, strasse as adresse, hausnummer, plz, ort, email, telefon_mobil
            FROM mitglieder m
            ${whereClause}
