...
router.put("/:id/beitrag", validateId('id'), async (req, res) => {
  ...
  try {
    ...
    // 2) Bereits generierte, noch unbezahlte künftige Monatsbeiträge anpassen,
    //    damit die Änderung auch im Lastschriftlauf wirksam wird.
...
// ── PUT /api/mitglieder/:id/stil-erinnerung-dismiss ── Als gesehen markieren ─
router.put('/:id/stil-erinnerung-dismiss', async (req, res) => {
  try {
    const mitgliedId = parseInt(req.params.id, 10);
    if (isNaN(mitgliedId)) return res.status(400).json({ error: 'Ungültige ID' });

    const dojoId = getSecureDojoId(req);
    const pool = db.promise();

    const dojoCondition = dojoId ? ' AND dojo_id = ?' : '';
    const params = dojoId ? [mitgliedId, dojoId] : [mitgliedId];

    await pool.query(
      `UPDATE mitglieder SET stil_erinnerung_dismissed = 1 WHERE mitglied_id = ?${dojoCondition}`,
      params
    );

    res.json({ success: true });
  } catch (err) {
    logger.error('Stil-Erinnerung Dismiss Fehler:', err);
    res.status(500).json({ error: 'Datenbankfehler' });
  }
});
