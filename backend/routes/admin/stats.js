  ...
  try {
    ...
    const [
      ...
      db.promise().query(`
        ...
        FROM mitglieder
      `),
      db.promise().query(`SELECT COUNT(*) as total_courses, COUNT(DISTINCT dojo_id) as dojos_with_courses FROM kurse`),
      db.promise().query(`SELECT COUNT(*) as total_trainers, COUNT(DISTINCT dojo_id) as dojos_with_trainers FROM trainer`),
      db.promise().query(`SELECT COUNT(*) as active_checkins_today FROM checkins WHERE DATE(checkin_time) = CURDATE() AND status = 'active'`),
      db.promise().query(`SELECT COUNT(*) as open_payments, SUM(betrag) as open_amount FROM beitraege WHERE bezahlt = 0`),
      db.promise().query(`
...
router.get('/tda-stats', requireSuperAdmin, async (req, res) => {
  try {
    ...
    const [[memberStats], [courseStats], [trainerStats], [checkinStats], [beitraegeStats]] = await Promise.all([
      db.promise().query(`
        SELECT COUNT(*) as total_members, SUM(CASE WHEN aktiv = 1 THEN 1 ELSE 0 END) as active_members
        FROM mitglieder WHERE dojo_id = ?
      `, [TDA_DOJO_ID]),
      db.promise().query('SELECT COUNT(*) as total_courses FROM kurse WHERE dojo_id = ?', [TDA_DOJO_ID]),
      db.promise().query('SELECT COUNT(*) as total_trainers FROM trainer WHERE dojo_id = ?', [TDA_DOJO_ID]),
      db.promise().query(`
    ...
    const stats = {
      ...
      trainers: trainerStats[0],
      checkins: checkinStats[0],
      payments: beitraegeStats[0]
    };
