...
router.get('/', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT stil_filter_aktiv FROM checkin_einstellungen WHERE dojo_id = ?', [d]);
    res.json({ success: true, stil_filter_aktiv: rows.length ? !!rows[0].stil_filter_aktiv : false });
...
router.put('/', async (req, res) => {
  try {
    ...
    const aktiv = req.body.stil_filter_aktiv ? 1 : 0;
    await pool.query(
      `INSERT INTO checkin_einstellungen (dojo_id, stil_filter_aktiv) VALUES (?, ?)
       ON DUPLICATE KEY UPDATE stil_filter_aktiv = VALUES(stil_filter_aktiv)`,
      [d, aktiv]
    );
    res.json({ success: true, stil_filter_aktiv: !!aktiv });
