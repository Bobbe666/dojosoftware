...
router.get('/', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT stil_filter_aktiv, alter_filter_aktiv, guertel_filter_aktiv FROM checkin_einstellungen WHERE dojo_id = ?', [d]);
    res.json({
      success: true,
      ...
      guertel_filter_aktiv: rows.length ? !!rows[0].guertel_filter_aktiv : false,
    });
...
router.put('/', requirePermission('einstellungen', 'bearbeiten'), async (req, res) => {
  try {
    ...
    const guertel = req.body.guertel_filter_aktiv ? 1 : 0;
    await pool.query(
      `INSERT INTO checkin_einstellungen (dojo_id, stil_filter_aktiv, alter_filter_aktiv, guertel_filter_aktiv) VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE stil_filter_aktiv = VALUES(stil_filter_aktiv), alter_filter_aktiv = VALUES(alter_filter_aktiv), guertel_filter_aktiv = VALUES(guertel_filter_aktiv)`,
      [d, stil, alter, guertel]
    );
    res.json({ success: true, stil_filter_aktiv: !!stil, alter_filter_aktiv: !!alter, guertel_filter_aktiv: !!guertel });
