...
module.exports = function generateEuerPdfHTML(data, dojoInfo, logoBase64 = null) {
  ...
  // Berechne Kategorie-Summen (Einnahmen)
  ...
  // Berechne Kategorie-Summen (Ausgaben)
  const ausgabenSummen = {};
  ausgabenKategorien.forEach(kat => {
    ausgabenSummen[kat.key] = monate.reduce((sum, m) => {
      return sum + (m.ausgaben?.[kat.key] || 0);
    }, 0);
  });

  // Filtere Ausgaben-Kategorien mit Werten > 0
  const aktiveAusgabenKategorien = ausgabenKategorien.filter(kat => ausgabenSummen[kat.key] > 0);
...
<body>
  ...
  ${aktiveAusgabenKategorien.length > 0 ? `
  <div class="income-breakdown" style="background: #fef2f2; border-color: #fecaca;">
    <h3 style="color: #991b1b;">Aufschlüsselung der Betriebsausgaben</h3>
    <div class="breakdown-grid">
      ${aktiveAusgabenKategorien.map(kat => `
        <div class="breakdown-item" style="border-color: #fecaca;">
          <span class="label" style="color: #991b1b;">${kat.label}</span>
          <span class="value" style="color: #991b1b;">${formatCurrency(ausgabenSummen[kat.key] || 0)} €</span>
        </div>
      `).join('')}
    </div>
  </div>
  ...
  <div class="section" style="page-break-before: auto;">
    <div class="section-title">Betriebsausgaben nach Kategorien und Monaten</div>
    <table>
      <thead>
        <tr>
          <th>Monat</th>
          ${aktiveAusgabenKategorien.map(kat => `<th class="amount">${kat.label}</th>`).join('')}
          <th class="amount">Gesamt</th>
        </tr>
      </thead>
      <tbody>
        ${monate.map((m, idx) => `
          <tr>
            <td>${monatsnamen[idx]}</td>
            ${aktiveAusgabenKategorien.map(kat => `
              <td class="amount">${formatCurrency(m.ausgaben?.[kat.key] || 0)} €</td>
            `).join('')}
            <td class="amount negative"><strong>${formatCurrency(m.ausgaben?.gesamt || 0)} €</strong></td>
          </tr>
        `).join('')}
        <tr class="total">
          <td><strong>SUMME</strong></td>
          ${aktiveAusgabenKategorien.map(kat => `
            <td class="amount">${formatCurrency(ausgabenSummen[kat.key] || 0)} €</td>
          `).join('')}
          <td class="amount"><strong>${formatCurrency(jahresSumme?.ausgaben_gesamt || 0)} €</strong></td>
        </tr>
      </tbody>
    </table>
  </div>
