...
// Berechne tatsächlichen Server-Speicherverbrauch
function getServerDiskUsage() {
  const { execSync } = require('child_process');
  try {
    // df gibt Festplattenauslastung - wir nehmen das Root-Filesystem
    const result = execSync('df -BG / | tail -1', { encoding: 'utf8' });
    // Format: /dev/xxx  197G   13G  176G   7% /
    const parts = result.trim().split(/\s+/);
    const totalGB = parseInt(parts[1]) || 0;  // z.B. "197G" -> 197
    const usedGB = parseInt(parts[2]) || 0;   // z.B. "13G" -> 13
    const percentUsed = parseInt(parts[4]) || 0; // z.B. "7%" -> 7

    return { totalGB, usedGB, percentUsed };
  } catch (error) {
    console.error('❌ Fehler beim Abrufen der Festplattenauslastung:', error);
    return { totalGB: 0, usedGB: 0, percentUsed: 0 };
  }
}
...
router.get('/global-stats', requireSuperAdmin, async (req, res) => {
  try {
    ...
    const diskUsage = getServerDiskUsage();
...
// GET /admin/activities - Letzte Aktivitäten abrufen
router.get('/activities', requireSuperAdmin, async (req, res) => {
  try {
    ...
    newDojos.forEach(dojo => {
      activities.push({
        id: `dojo-${dojo.id}`,
        typ: 'dojo_registriert',
        titel: 'Neues Dojo registriert',
        beschreibung: `${dojo.dojoname} (${dojo.inhaber})`,
        details: dojo,
    ...
    try {
      ...
      newBestellungen.forEach(bestellung => {
        activities.push({
          id: `best-${bestellung.id}`,
          typ: 'bestellung',
          titel: 'Neue Bestellung',
          beschreibung: `${bestellung.bestellnummer} von ${bestellung.kunde_name || 'Unbekannt'} - ${(bestellung.gesamtbetrag_cent / 100).toFixed(2)}€`,
          details: bestellung,
          erstellt_am: bestellung.bestellt_am,
          icon: '🛒'
        });
      });
    } catch (e) {
    ...
    activities.sort((a, b) => new Date(b.erstellt_am) - new Date(a.erstellt_am));
...
router.get('/notifications', requireSuperAdmin, async (req, res) => {
  try {
    ...
    // Prüfe ob Tabelle existiert, wenn nicht erstellen
    await db.promise().query(`
    ...
    let whereClause = '1=1';
    if (filter === 'unread') {
      whereClause = 'gelesen = FALSE AND archiviert = FALSE';
    } else if (filter === 'archived') {
      whereClause = 'archiviert = TRUE';
    } else if (filter === 'all') {
      whereClause = 'archiviert = FALSE';
    }

    const [notifications] = await db.promise().query(`
      SELECT * FROM super_admin_notifications
      WHERE ${whereClause}
      ORDER BY erstellt_am DESC
      LIMIT 50
    `);
    ...
    const [unreadResult] = await db.promise().query(`
      SELECT COUNT(*) as count FROM super_admin_notifications
      WHERE gelesen = FALSE AND archiviert = FALSE
    `);
...
// PUT /admin/notifications/:id/read - Als gelesen markieren
router.put('/notifications/:id/read', requireSuperAdmin, async (req, res) => {
  try {
    ...
    console.error('❌ Fehler beim Markieren als gelesen:', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// PUT /admin/notifications/:id/archive - Archivieren
router.put('/notifications/:id/archive', requireSuperAdmin, async (req, res) => {
  try {
    const { id } = req.params;

    await db.promise().query(`
      UPDATE super_admin_notifications
      SET archiviert = TRUE, archiviert_am = NOW(), gelesen = TRUE
      WHERE id = ?
    `, [id]);

    res.json({ success: true });
  } catch (err) {
    console.error('❌ Fehler beim Archivieren:', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// PUT /admin/notifications/mark-all-read - Alle als gelesen markieren
router.put('/notifications/mark-all-read', requireSuperAdmin, async (req, res) => {
  try {
    ...
    console.error('❌ Fehler beim Markieren aller als gelesen:', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// POST /admin/push-notifications/send - Push-Nachricht senden
router.post('/push-notifications/send', requireSuperAdmin, async (req, res) => {
  try {
    const { titel, nachricht, empfaenger_typ, prioritaet = 'normal' } = req.body;
    ...
    // Speichere die Benachrichtigung in der Super-Admin-Tabelle
    await db.promise().query(`
      INSERT INTO super_admin_notifications (typ, titel, nachricht, prioritaet, empfaenger_typ)
      VALUES ('push_gesendet', ?, ?, ?, ?)
    `, [titel, nachricht, prioritaet, empfaenger_typ]);

    // Zähle potenzielle Empfänger basierend auf Typ
    let recipientCount = 0;

    if (empfaenger_typ === 'alle' || empfaenger_typ === 'verbandsmitglieder') {
      const [vm] = await db.promise().query('SELECT COUNT(*) as count FROM verband_mitglieder WHERE ist_aktiv = 1');
      recipientCount += vm[0].count;
    }

    if (empfaenger_typ === 'alle' || empfaenger_typ === 'dojos') {
      const [dojos] = await db.promise().query('SELECT COUNT(*) as count FROM dojo WHERE ist_aktiv = 1');
      recipientCount += dojos[0].count;
    }

    if (empfaenger_typ === 'alle' || empfaenger_typ === 'mitglieder') {
      const [mitglieder] = await db.promise().query('SELECT COUNT(*) as count FROM mitglieder WHERE ist_aktiv = 1');
      recipientCount += mitglieder[0].count;
    }

    // Speichere auch in einer Benachrichtigungen-Tabelle für alle Empfänger (wenn vorhanden)
    try {
      // Für Verbandsmitglieder: Speichere in deren Benachrichtigungen
      if (empfaenger_typ === 'alle' || empfaenger_typ === 'verbandsmitglieder') {
        await db.promise().query(`
        ...
        // Füge Benachrichtigung für alle aktiven Verbandsmitglieder ein
        await db.promise().query(`
          INSERT INTO verband_mitglied_notifications (verband_mitglied_id, typ, titel, nachricht, prioritaet)
          SELECT id, 'push', ?, ?, ?
          FROM verband_mitglieder
          WHERE ist_aktiv = 1
    ...
    res.json({
      success: true,
      message: `Push-Nachricht erfolgreich gesendet`,
      recipientCount
    });
  } catch (err) {
    console.error('❌ Fehler beim Senden der Push-Nachricht:', err.message);
    res.status(500).json({
      success: false,
      error: 'Fehler beim Senden der Push-Nachricht'
    });
  }
});
