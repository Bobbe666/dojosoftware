...
router.post('/login',
  ...
  try {
    ...
    // COALESCE: falls users.dojo_id/mitglied_id NULL ist, über E-Mail-Match aus mitglieder holen
    const userQuery = `
      SELECT u.id, u.username, u.email, u.password, u.role,
             COALESCE(u.dojo_id, m.dojo_id, m2.dojo_id) as dojo_id,
             COALESCE(u.mitglied_id, m2.mitglied_id) as mitglied_id,
             u.created_at, u.msg_app_enabled
      FROM users u
      LEFT JOIN mitglieder m  ON u.mitglied_id = m.mitglied_id
      LEFT JOIN mitglieder m2 ON u.email = m2.email AND u.mitglied_id IS NULL AND m2.aktiv = 1
    ...
    const adminQuery = `
      SELECT id, username, email, password, password_algorithm, rolle as role, dojo_id,
             vorname, nachname, berechtigungen, aktiv, erstellt_am, trainer_id,
    ...
    async function processLogin(user, password, res, req, isAdmin, clientInfo) {
      try {
        ...
        const tokenPayload = {
          ...
          trainer_id: isAdmin ? (user.trainer_id || null) : null,
