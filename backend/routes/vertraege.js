...
// GET /api/vertraege/:id/kuendigungsbestaetigung - Kuendigungsbestaetigung als PDF generieren
router.get('/:id/kuendigungsbestaetigung', async (req, res) => {
    try {
        const { id } = req.params;

        // 1. Lade Vertragsdaten mit allen notwendigen Informationen
        const vertragResults = await queryAsync(`
            SELECT v.*,
                   m.vorname, m.nachname, m.email, m.geburtsdatum,
                   m.strasse, m.hausnummer, m.plz, m.ort, m.telefon,
        ...
        // 3. Erstelle Datenobjekte fuer PDF-Generator
        const mitgliedData = {
            vorname: vertrag.vorname,
            nachname: vertrag.nachname,
            anrede: vertrag.anrede,
        ...
        const vertragData = {
            ...
            vertragsbeginn: vertrag.vertragsbeginn,
            vertragsende: vertrag.vertragsende,
            kuendigung_eingegangen: vertrag.kuendigung_eingegangen,
            kuendigungsdatum: vertrag.kuendigungsdatum,
            kuendigungsgrund: vertrag.kuendigungsgrund,
        ...
        console.error('Fehler beim Generieren der Kuendigungsbestaetigung:', err);
        res.status(500).json({ error: 'Fehler beim Generieren der Kuendigungsbestaetigung', details: err.message });
    }
});

// POST /api/vertraege/:id/kuendigungsbestaetigung/speichern - Kuendigungsbestaetigung speichern und optional per E-Mail senden
router.post('/:id/kuendigungsbestaetigung/speichern', async (req, res) => {
    try {
        const { id } = req.params;
        const { send_email } = req.body;

        // 1. Lade Vertragsdaten
        const vertragResults = await queryAsync(`
            SELECT v.*,
                   m.vorname, m.nachname, m.email, m.geburtsdatum,
                   m.strasse, m.hausnummer, m.plz, m.ort, m.telefon,
        ...
        // 4. Speichere PDF als Mitglieder-Dokument
        const docsDir = await ensureDocumentsDir();
        const timestamp = Date.now();
        ...
        // 5. Speichere in mitglied_dokumente
        ...
        const dokumentResult = await queryAsync(insertQuery, [
            vertrag.mitglied_id,
            vertrag.dojo_id,
            dokumentname,
            relativePath
        ...
        try {
            ...
            console.warn('Fehler beim Erstellen des Historie-Eintrags:', histErr.message);
        }

        // 7. Optional: E-Mail senden
        let emailSent = false;
        if (send_email && vertrag.email) {
            try {
                // Hier koennte die E-Mail-Logik eingefuegt werden
                // emailSent = await sendKuendigungsbestaetigungEmail({ ... });
                console.log(`E-Mail-Versand fuer Kuendigungsbestaetigung an ${vertrag.email} (noch nicht implementiert)`);
            } catch (emailErr) {
        ...
        res.json({
            ...
            data: {
                dokument_id: dokumentResult.insertId,
                filename: filename,
                filepath: relativePath,
                email_sent: emailSent
            }
        ...
        console.error('Fehler beim Speichern der Kuendigungsbestaetigung:', err);
