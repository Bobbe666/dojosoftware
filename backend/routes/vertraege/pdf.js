...
router.get('/:id/kuendigungsbestaetigung', async (req, res) => {
  try {
    ...
    const vertragResults = await queryAsync(`
      SELECT v.*, m.vorname, m.nachname, m.email, m.geburtsdatum, m.strasse, m.hausnummer, m.plz, m.ort,
        m.telefon, CASE WHEN m.geschlecht='w' THEN 'Frau' WHEN m.geschlecht='m' THEN 'Herr' ELSE '' END AS anrede, m.mitglied_id AS mitgliedsnummer, t.name as tarif_name
      FROM vertraege v
      LEFT JOIN mitglieder m ON v.mitglied_id = m.mitglied_id
      LEFT JOIN tarife t ON v.tarif_id = t.id
      WHERE v.id = ? ${secureDojoId !== null ? 'AND v.dojo_id = ?' : ''}
...
router.post('/:id/kuendigungsbestaetigung/speichern', async (req, res) => {
  try {
    ...
    const vertragResults = await queryAsync(`
      SELECT v.*, m.vorname, m.nachname, m.email, m.geburtsdatum, m.strasse, m.hausnummer, m.plz, m.ort,
        m.telefon, CASE WHEN m.geschlecht='w' THEN 'Frau' WHEN m.geschlecht='m' THEN 'Herr' ELSE '' END AS anrede, m.mitglied_id AS mitgliedsnummer
      FROM vertraege v
      LEFT JOIN mitglieder m ON v.mitglied_id = m.mitglied_id
      WHERE v.id = ? ${secureDojoId !== null ? 'AND v.dojo_id = ?' : ''}
    `, secureDojoId !== null ? [id, secureDojoId] : [id]);
