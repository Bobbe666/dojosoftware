-- Migration 151: euer_ausblenden Flag für beitraege
-- Ermöglicht das Ausblenden von Beitrag-Einträgen aus der EÜR-View
-- ohne die Datensätze zu löschen (z.B. bei doppelter Erfassung durch Bank-Import)

ALTER TABLE beitraege ADD COLUMN euer_ausblenden TINYINT(1) NOT NULL DEFAULT 0;

CREATE INDEX idx_beitraege_euer ON beitraege (euer_ausblenden, bezahlt, dojo_id);
...
CREATE OR REPLACE VIEW v_euer_einnahmen AS
  ...
  WHERE b.buchungsart = 'einnahme'
    AND b.storniert = 0
    AND b.kategorie NOT IN ('privateinlage', 'privatentnahme')

  UNION ALL

  SELECT 'Beitrag' AS quelle,
  ...
  WHERE b.bezahlt = 1
    ...
    AND b.euer_ausblenden = 0

  UNION ALL

  SELECT 'Rechnung' AS quelle,
         r.rechnung_id AS referenz_id,
         COALESCE(r.dojo_id, 1) AS dojo_id,
         ...
         COALESCE(r.brutto_betrag, r.betrag) AS betrag_brutto,
         'betriebseinnahmen' AS kategorie,
  ...
  SELECT 'Bank' AS quelle,
         ...
         COALESCE(bt.kategorie, 'betriebseinnahmen') AS kategorie,
