...
router.put('/dojos/:id/activate-subscription', requireSuperAdmin, async (req, res) => {
  try {
    ...
    // dojo_subscriptions.feature_* Flags aus plan_feature_mapping synchronisieren
...
router.put('/plans/:planName/features', requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    // Alle Dojos mit diesem Plan re-syncen
    const [affectedDojos] = await db.promise().query(
      `SELECT dojo_id FROM dojo_subscriptions WHERE plan_type = ?`,
      [planName]
    );
    for (const { dojo_id } of affectedDojos) {
      await syncPlanFeatures(dojo_id, planName);
    }

    logger.info('Plan-Features aktualisiert:', { planName, planId, featureKeys: features, featureCount: featureIds.length, dojosResynced: affectedDojos.length });
    res.json({
      success: true,
      ...
      dojos_resynced: affectedDojos.length
    });
  } catch (error) {
