    ...
    async processLastschriftBatch(mitgliederMitBeitraegen, monat, jahr) {
        ...
        for (const item of mitgliederMitBeitraegen) {
            try {
                ...
                // Verwendungszweck aus den Posten zusammensetzen, damit das Mitglied
                // auf dem Kontoauszug sieht, wofür abgebucht wurde (nur Text).
                let description = `Mitgliedsbeitrag ${String(monat).padStart(2, '0')}/${jahr}`;
                try {
                    if (beitragIds.length > 0) {
                        const ph = beitragIds.map(() => '?').join(',');
                        const arts = await queryAsync(
                            `SELECT art, COUNT(*) AS c FROM beitraege WHERE beitrag_id IN (${ph}) GROUP BY art`,
                            beitragIds
                        );
                ...
                } catch (_) { /* Fallback: Standard-Beschreibung */ }
