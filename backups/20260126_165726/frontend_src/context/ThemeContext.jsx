...
/**
 * useForceDarkTheme Hook
 * Erzwingt dunkles Theme auf öffentlichen Seiten (Login, Home, etc.)
 * Stellt beim Verlassen das ursprüngliche Theme wieder her
 */
export const useForceDarkTheme = () => {
  const { theme, setTheme, isDarkMode } = useTheme();

  useEffect(() => {
    // Speichere aktuelles Theme falls wir es wiederherstellen müssen
    const originalTheme = theme;

    // Wenn nicht bereits im Dark Mode, wechsle zu midnight
    if (!isDarkMode) {
      // Setze direkt ohne Animation für sofortigen Effekt
      document.documentElement.setAttribute('data-theme', 'midnight');
    }

    // Beim Verlassen der Seite: Ursprüngliches Theme wiederherstellen
    return () => {
      if (!THEMES[originalTheme]?.isDark) {
        // Nur wiederherstellen wenn Original hell war
