...
const Finanzcockpit = () => {
  ...
  const [aufnahmegebuehrData, setAufnahmegebuehrData] = useState(null);
  const [vertragsfreiData, setVertragsfreiData] = useState(null);
  const [tariferhohungData, setTariferhohungData] = useState(null);
  const [letzterMonatData, setLetzterMonatData] = useState(null);

  useEffect(() => {
    loadFinanzStats();
    loadTimelineData();
    loadTarifData();
    loadAufnahmegebuehrData();
    loadVertragsfreiData();
    loadTariferhohungData();
    loadLetzterMonatData();
  ...
  const loadAufnahmegebuehrData = async () => {
    try {
      const dojoFilterParam = getDojoFilterParam();
      const separator = dojoFilterParam ? '&' : '';
      const response = await fetchWithAuth(`${config.apiBaseUrl}/finanzcockpit/aufnahmegebuehren?period=${selectedPeriod}${separator}${dojoFilterParam}`);

      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          setAufnahmegebuehrData(result.data);
        }
      }
    } catch (error) {
      console.error('Fehler beim Laden der Aufnahmegebühren:', error);
    }
  };

  const loadVertragsfreiData = async () => {
    try {
      const dojoFilterParam = getDojoFilterParam();
      const response = await fetchWithAuth(`${config.apiBaseUrl}/finanzcockpit/vertragsfrei?${dojoFilterParam}`);

      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          setVertragsfreiData(result.data);
        }
      }
    } catch (error) {
      console.error('Fehler beim Laden der Vertragsfrei-Daten:', error);
    }
  };

  const loadTariferhohungData = async () => {
    try {
      const dojoFilterParam = getDojoFilterParam();
      ...
      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          setTariferhohungData(result.data);
        }
      }
    } catch (error) {
      console.error('Fehler beim Laden der Tariferhöhungs-Daten:', error);
    }
  };

  const loadLetzterMonatData = async () => {
    try {
      const dojoFilterParam = getDojoFilterParam();
      const response = await fetchWithAuth(`${config.apiBaseUrl}/finanzcockpit/letzter-monat?${dojoFilterParam}`);

      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          setLetzterMonatData(result.data);
        }
      }
    } catch (error) {
      console.error('Fehler beim Laden der Letzter-Monat-Daten:', error);
    }
  };
  ...
  // Daten für Einnahmen-Aufschlüsselung (inkl. Aufnahmegebühren)
  const incomeBreakdownData = stats ? [
    ...
    { name: 'Aufnahme', value: aufnahmegebuehrData?.aufnahmegebuehrenEinnahmen || 0 }
  ].filter(item => item.value > 0) : [];
  ...
  const kpiCards = stats ? [
    ...
    },
    {
      ...
      value: formatCurrency(aufnahmegebuehrData?.aufnahmegebuehrenEinnahmen || 0),
      detail: `${aufnahmegebuehrData?.neueVertraege || 0} neue Verträge`,
      tone: 'positive'
    },
    {
      id: 'vertragsfrei',
      icon: AlertCircle,
      label: 'Ohne Vertrag',
      value: formatNumber(vertragsfreiData?.anzahl || 0),
      detail: 'Mitglieder ohne aktiven Vertrag',
      tone: (vertragsfreiData?.anzahl || 0) > 0 ? 'warning' : 'positive'
    },
    {
      id: 'tariferhohung',
      icon: TrendingUp,
      label: 'Erhöhungspotential',
      value: formatCurrency(tariferhohungData?.monatlichesPotential || 0),
      detail: `${tariferhohungData?.anzahl || 0} Mitglieder unter Tarifpreis`,
      tone: (tariferhohungData?.anzahl || 0) > 0 ? 'warning' : 'positive'
    }
  ] : [];
    ...
    <div className="finanzcockpit">
      ...
      <section className="finanzcockpit__layout">
        <div className="finanzcockpit__main">
          ...
          {/* Tarif-Aufschlüsselung Grafik */}
          {tarifData.length > 0 && (
            <div className="finanzcockpit-card finanzcockpit__chart-card">
              <div className="finanzcockpit__card-header">
                <div>
                  <span className="finanzcockpit__card-eyebrow">Tarife</span>
                  <h3>Einnahmen nach Tarifen</h3>
                </div>
                <PieChartIcon size={18} />
              </div>
              <ResponsiveContainer width="100%" height={320}>
                <PieChart>
                  ...
                  >
                    {tarifData.map((entry, index) => (
                      <Cell key={`tarif-cell-${index}`} fill={COLORS[index % COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip
                    contentStyle={{
                    ...
                    formatter={(value, name, entry) => [
                      formatCurrency(value) + ` (${entry.payload.count} Verträge)`,
                      name
                    ]}
                  />
                  <Legend />
      ...
      {/* Letzter Monat Übersicht */}
      {letzterMonatData && (
        <section className="finanzcockpit__letzter-monat">
          <div className="finanzcockpit-card">
            <div className="finanzcockpit__card-header">
              <div>
                <span className="finanzcockpit__card-eyebrow">Rückblick</span>
                <h3>Letzter Monat: {letzterMonatData.monat}</h3>
              </div>
              <div style={{
                display: 'flex',
                alignItems: 'center',
                ...
                padding: '0.5rem 1rem',
                borderRadius: '8px',
                backgroundColor: letzterMonatData.erfuellungsquote >= 100 ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                color: letzterMonatData.erfuellungsquote >= 100 ? '#10b981' : '#ef4444'
              }}>
                {letzterMonatData.erfuellungsquote >= 100 ? <CheckCircle2 size={18} /> : <AlertCircle size={18} />}
                <span style={{ fontWeight: '600' }}>{letzterMonatData.erfuellungsquote}% Erfüllung</span>
              </div>
            </div>

            <div className="finanzcockpit__letzter-monat-grid" style={{
              display: 'grid',
              gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))',
              gap: '1.5rem',
              marginTop: '1.5rem'
            }}>
              <div className="finanzcockpit__lm-stat">
                <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>Erwartete Einnahmen</span>
                <span style={{ fontSize: '1.5rem', fontWeight: '700', color: '#f59e0b' }}>
                  {formatCurrency(letzterMonatData.erwarteteEinnahmen)}
                </span>
                <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)' }}>
                  {letzterMonatData.anzahlVertraege} aktive Verträge
                </span>
              </div>

              <div className="finanzcockpit__lm-stat">
                <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>Tatsächliche Einnahmen</span>
                <span style={{ fontSize: '1.5rem', fontWeight: '700', color: '#10b981' }}>
                  {formatCurrency(letzterMonatData.gesamtEinnahmen)}
                </span>
                <span style={{
                  fontSize: '0.8rem',
                  color: letzterMonatData.differenz >= 0 ? '#10b981' : '#ef4444'
                }}>
                  {letzterMonatData.differenz >= 0 ? '+' : ''}{formatCurrency(letzterMonatData.differenz)} Differenz
                </span>
              </div>

              <div className="finanzcockpit__lm-stat">
                <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>Zahlungen & Zahlläufe</span>
                <span style={{ fontSize: '1.5rem', fontWeight: '700' }}>
                  {formatCurrency(letzterMonatData.tatsaechlicheZahlungen + letzterMonatData.zahllaeufeEinnahmen)}
                </span>
                <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)' }}>
                  {letzterMonatData.anzahlZahlungen} Zahlungen, {letzterMonatData.anzahlZahllaeufe} Zahlläufe
                </span>
              </div>

              <div className="finanzcockpit__lm-stat">
                <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>Aufnahmegebühren</span>
                <span style={{ fontSize: '1.5rem', fontWeight: '700', color: '#8b5cf6' }}>
                  {formatCurrency(letzterMonatData.aufnahmegebuehren)}
                </span>
                <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)' }}>
                  {letzterMonatData.neueVertraege} neue Verträge
                </span>
              </div>

              <div className="finanzcockpit__lm-stat">
                <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem' }}>Verkäufe</span>
                <span style={{ fontSize: '1.5rem', fontWeight: '700', color: '#3b82f6' }}>
                  {formatCurrency(letzterMonatData.verkaufEinnahmen)}
                </span>
                <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)' }}>
                  {letzterMonatData.anzahlVerkaeufe} Verkäufe
                </span>
              </div>
            </div>
          </div>
      ...
      <section className="finanzcockpit__actions">
        ...
        <div className="finanzcockpit__actions-grid">
          ...
          <button
            className="finanzcockpit__action-button"
            onClick={() => navigate('/dashboard/tariferhohung')}
