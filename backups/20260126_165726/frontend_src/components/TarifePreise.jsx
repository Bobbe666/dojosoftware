...
const TarifePreise = () => {
  ...
  const getDojoId = () => {
    if (!activeDojo || activeDojo === 'super-admin') {
      return 'all'; // Super-Admin sieht alle Dojos
    }
  ...
  const loadTarifeUndRabatte = async () => {
    try {
      ...
      // Dojo-Filter aus Context - berücksichtigt "Alle Dojos" Filter
      const filterParam = getDojoFilterParam();
      const dojoParam = filterParam ? `?${filterParam}` : '?dojo_id=all';

      console.log('📊 TarifePreise: Lade Tarife mit Filter:', filterParam, 'activeDojo:', activeDojo);
  ...
  // Handler für Nachfolger-Tarif setzen
  const handleSetNachfolger = async (tarifId, nachfolgerTarifId) => {
    try {
      const response = await axios.patch(`/tarife/${tarifId}/nachfolger`, {
        nachfolger_tarif_id: nachfolgerTarifId || null
      });

      if (response.data.success) {
        await loadTarifeUndRabatte();
      } else {
        alert('Fehler beim Setzen des Nachfolger-Tarifs: ' + response.data.error);
      }
    } catch (error) {
      console.error('Fehler beim Setzen des Nachfolger-Tarifs:', error);
      alert('Fehler beim Setzen des Nachfolger-Tarifs');
    }
  };
                ...
                {tarife.filter(tarif => tarif.ist_archiviert).map(tarif => (
                  <div key={tarif.id} className="tarif-card" style={{ opacity: 0.7, border: '2px solid #6b7280' }}>
                    ...
                    <div className="tarif-details">
                      ...
                      {/* Nachfolger-Tarif Auswahl */}
                      <div className="detail-item" style={{ gridColumn: '1 / -1', marginTop: '0.5rem', borderTop: '1px solid #e5e7eb', paddingTop: '0.75rem' }}>
                        <span className="label" style={{ color: '#f59e0b', fontWeight: '600' }}>
                          🔄 Nachfolger-Tarif
                        </span>
                        <select
                          value={tarif.nachfolger_tarif_id || ''}
                          onChange={(e) => handleSetNachfolger(tarif.id, e.target.value ? parseInt(e.target.value) : null)}
                          style={{
                            width: '100%',
                            padding: '0.5rem',
                            borderRadius: '6px',
                            border: tarif.nachfolger_tarif_id ? '2px solid #10b981' : '2px solid #f59e0b',
                            background: tarif.nachfolger_tarif_id ? 'rgba(16, 185, 129, 0.1)' : 'rgba(245, 158, 11, 0.1)',
                            fontSize: '0.875rem',
                            marginTop: '0.25rem'
                          }}
                        >
                          <option value="">-- Nachfolger wählen --</option>
                          {tarife.filter(t => !t.ist_archiviert && t.id !== tarif.id).map(t => (
                            <option key={t.id} value={t.id}>
                              {t.name} (€{t.price_euros}/Monat)
                            </option>
                          ))}
                        </select>
                        {!tarif.nachfolger_tarif_id && (
                          <p style={{ fontSize: '0.75rem', color: '#f59e0b', marginTop: '0.25rem', fontStyle: 'italic' }}>
                            ⚠️ Ohne Nachfolger-Tarif können Beiträge nicht automatisch angepasst werden
                          </p>
                        )}
                        {tarif.nachfolger_tarif_id && (
                          <p style={{ fontSize: '0.75rem', color: '#10b981', marginTop: '0.25rem' }}>
                            ✓ Mitglieder mit diesem Tarif werden auf den Nachfolger-Preis angepasst
                          </p>
                        )}
                      </div>
                    </div>
