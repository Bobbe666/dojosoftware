...
-- 3. Lösche alle Zahlungen von MagicLine-Mitgliedern
DELETE FROM beitraege
WHERE mitglied_id IN (
  SELECT mitglied_id FROM mitglieder
  WHERE magicline_customer_number IS NOT NULL
)
OR magicline_transaction_id IS NOT NULL;

-- 4. Lösche alle Verträge von MagicLine-Mitgliedern
...
-- 5. Lösche alle MagicLine-Mitglieder
...
SELECT
  ...
  (SELECT COUNT(*) FROM beitraege WHERE magicline_transaction_id IS NOT NULL) as remaining_payments;
