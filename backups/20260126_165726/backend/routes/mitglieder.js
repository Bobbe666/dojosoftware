...
router.post("/", async (req, res) => {
    ...
    db.query(query, values, async (err, result) => {
        ...
        if (memberData.vertrag_tarif_id) {
            ...
            // AUTO: Tarifpreise und Aufnahmegebühr laden wenn nicht explizit übergeben
            let autoAufnahmegebuehr = memberData.vertrag_aufnahmegebuehr_cents;
            let autoMonatsbeitrag = memberData.vertrag_monatsbeitrag;
            if (autoAufnahmegebuehr === undefined || autoMonatsbeitrag === undefined) {
                try {
                    const tarifResult = await queryAsync("SELECT price_cents, aufnahmegebuehr_cents FROM tarife WHERE id = ?", [parseInt(memberData.vertrag_tarif_id)]);
                    if (tarifResult.length > 0) {
                        if (autoAufnahmegebuehr === undefined && tarifResult[0].aufnahmegebuehr_cents) {
                            autoAufnahmegebuehr = tarifResult[0].aufnahmegebuehr_cents;
                            console.log("[Mitglied] Aufnahmegebühr automatisch vom Tarif übernommen:", autoAufnahmegebuehr, "Cent");
                        }
                        if (autoMonatsbeitrag === undefined && tarifResult[0].price_cents) {
                            autoMonatsbeitrag = tarifResult[0].price_cents / 100;
                            console.log("[Mitglied] Monatsbeitrag automatisch vom Tarif übernommen:", autoMonatsbeitrag, "EUR");
                        }
                    }
                } catch (tarifErr) {
                    console.error("[Mitglied] Fehler beim Laden der Tarif-Daten:", tarifErr);
                }
            }
            ...
            const vertragData = {
                ...
                monatsbeitrag: autoMonatsbeitrag || null,
                aufnahmegebuehr_cents: autoAufnahmegebuehr || null,
                billing_cycle: memberData.vertrag_billing_cycle || 'monthly',
                payment_method: memberData.vertrag_payment_method || 'direct_debit',
                vertragsbeginn: memberData.vertrag_vertragsbeginn || new Date().toISOString().split('T')[0],
