...
console.log('📦 Starte Rechnungen Tabellen Migration...');

// Erstelle Rechnungen Tabelle
const createRechnungenQuery = `
...
db.query(createRechnungenQuery, (err) => {
  if (err) {
    console.error('❌ Fehler beim Erstellen der Rechnungen Tabelle:', err);
    process.exit(1);
  }

  console.log('✅ Rechnungen Tabelle erstellt!');

  // Erstelle Rechnungspositionen Tabelle
  const createPositionenQuery = `
  ...
  db.query(createPositionenQuery, (posErr) => {
    if (posErr) {
      console.error('❌ Fehler beim Erstellen der Rechnungspositionen Tabelle:', posErr);
      process.exit(1);
    }

    console.log('✅ Rechnungspositionen Tabelle erstellt!');

    // Erstelle Zahlungen Tabelle
    const createZahlungenQuery = `
    ...
    db.query(createZahlungenQuery, (zahlErr) => {
      if (zahlErr) {
        console.error('❌ Fehler beim Erstellen der Zahlungen Tabelle:', zahlErr);
        process.exit(1);
      }

      console.log('✅ Zahlungen Tabelle erstellt!');
